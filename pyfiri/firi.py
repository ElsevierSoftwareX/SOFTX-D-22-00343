...
class firi2018:
    ...
    def get_profile(self, doy, chi, lat, f10_7):
        ...
        :param doy: (int) or (1D array_like int); day of year, (0 - 365)
        ...
        for _ in doy:
            if (_ > 365) or (_ < 1):
                raise ValueError("Accepted Day Number should be in [1..365]; "
        ...
        for _ in lat:
            if (_ > 60) or (_ < 0):
                raise ValueError("Latitude should be in [0..60] range."
...
class firi2018ext:
    def __init__(self):
        self._firi_data = _firi2018.get_data_extrapolated()
    ...
    def asDataArray(self):
        """
        'asDataArray' method the underlying extrapolated DataArray back to the user.
        Extra DOYs were added (copied from DOYs 350, and 15) to the original
        data-values by O.Z. to make the standard interpolation (DataArray method
        interp) work correctly with DOYs prior to DOY=15 and exceeding DOY=350.
        :return: xarray.DataArray with extrapolated FIRI-2018 electron density
                 values (reference profiles)
        """
        return self._firi_data.copy(deep=True)
    ...
    def get_profile(self, doy, chi, lat, f10_7):
        ...
        (vertical profile) for a given latitude and specified conditions.
        The profile is either one from tabulated (pre-calculated) values
        or is obtained via interpolation.
        :param doy: (int) or (1D array_like int); day of year, (1 - 366)
        :param chi: (float) or (1D array_like float); Solar zenith angle, [deg.]
        :param lat: (float) or (1D array_like float); latitude, [deg.]
        :param f10_7: (float) or (1D array_like float); Solar F10.7 index, [s.f.u.]
        :return: xarray.ArrayData; vertical electron density values, [el./(m**3)], with associated metadata
        ...
        # Check for input parameters' ranges
        # ? doy is in 1 - 366 range
        for _ in doy:
            if (_ > 366) or (_ < 1):
                raise ValueError("Accepted Day Number should be in [1..366]; "
                                 "The value {value} is given."
                                 .format(value=_))
        ...
        # ? latitude is in 0 - 60 range
        for _ in lat:
            if (_ > 60) or (_ < -60):
                raise ValueError("Latitude should be in [-60..60] range."
                                 "A user should fit input values into [-60..60] range if required."
                                 "The value {value} is given."
                                 .format(value=_))
    ...
    def interp(self, *args, **kwargs):
        """
        Method 'interp' is a direct proxy
                to the interp method
                of the underlying xarray.DataArray
                (model tabulated values).
        :param args: positional arguments to pass
        :param kwargs: key-value arguments to pass
        :return: DataArray with interpolated values
